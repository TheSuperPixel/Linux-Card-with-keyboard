...
#define MY_X_OFFSET 0
#define MY_Y_OFFSET 34
static void fbtft_set_addr_win(struct fbtft_par *par, int xs, int ys, int xe,
			       int ye)
{
	write_reg(par, MIPI_DCS_SET_COLUMN_ADDRESS, (xs + MY_X_OFFSET) >> 8,
		  xs + MY_X_OFFSET, ((xe + MY_X_OFFSET) >> 8) & 0xFF,
		  (xe + MY_X_OFFSET) & 0xFF);

	write_reg(par, MIPI_DCS_SET_PAGE_ADDRESS,
		  ((ys + MY_Y_OFFSET) >> 8) & 0xFF, (ys + MY_Y_OFFSET) & 0xFF,
		  ((ye + MY_Y_OFFSET) >> 8) & 0xFF, (ye + MY_Y_OFFSET) & 0xFF);
